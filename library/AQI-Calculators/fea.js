// AQI Calculator for Germany (FEA Standard)

// Source: Umweltbundesamt (German Environmental Agency)
...
module.exports = (pollutantValues) => {
    ...
    const pollutants = [
        { 
            ...
            I_LO: [0, 21, 41, 101, 201], 
            I_HI: [20, 40, 100, 200, 300] 
        },
        { 
            name: 'PM10', 
        ...
        if (value !== undefined && !isNaN(value)) {
            for (let i = 0; i < pollutant.BP_LO.length; i++) {
                if (value >= pollutant.BP_LO[i] && value <= pollutant.BP_HI[i]) {
                    ...
                    const I_P = ((pollutant.I_HI[i] - pollutant.I_LO[i]) / (pollutant.BP_HI[i] - pollutant.BP_LO[i])) * 
